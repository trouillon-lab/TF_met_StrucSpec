...
if [ -n "$JOB1" ]; then
    ...
    JOB2=$(sbatch --parsable --dependency=afterok:$JOB1 --job-name=af3_stage2_msas --output=logs/stage2_msas_%j.out --error=logs/stage2_msas_%j.err --time=04:00:00 --cpus-per-task=1 --mem-per-cpu=1G --wrap="batch-infer start alphafold3_datafill_msas")
